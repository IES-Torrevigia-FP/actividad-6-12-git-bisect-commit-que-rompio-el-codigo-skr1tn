...
Extracto Reflexion 6.12 (max 8k chars, si existe):
```
{reflex_excerpt}
```
...
- Esta actividad (6.12) se califica con rubrica: Bisect manual (max 4), Bisect run opcional (max 3), Reflexion (max 3).
- No incluyas README como criterio de calificacion ni como area de mejora principal.
- Si en JSON aparece bisect_checks.cp2_prueba_manual_good_bad.ok en true, reconoce explicitamente la prueba manual good/bad.
- Si en JSON aparece bisect_checks.cp3_proceso_bisect_manual.ok en true, reconoce explicitamente el uso de `git bisect start`, `git bisect good`, `git bisect bad` y `git bisect reset`.
- Si en JSON aparece bisect_checks.cp4_bisect_run_opcional.ok en true, reconoce explicitamente el uso de `git bisect run`.
- Espanol formal.
...
def main():
    ...
    reflex_excerpt = read_opt(Path('reflexion-6-12.md'))
