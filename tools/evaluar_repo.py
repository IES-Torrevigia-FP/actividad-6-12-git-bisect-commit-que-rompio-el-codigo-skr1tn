...
def analyze_bisect_activity(root: Path) -> dict:
    cp1_path = root / 'cp1-historial-bisect.txt'
    cp2_path = root / 'cp2-prueba-manual.txt'
    cp3_path = root / 'cp3-proceso-bisect.txt'
    cp4_path = root / 'cp4-bisect-run.txt'
    contador_path = root / 'contador.py'
    test_script_path = root / 'test_contador.sh'
    ...
    contador = safe_read_text(contador_path).lower()

    cp1_historial_ok = contains_any(cp1, ['git log --oneline']) or contains_any(cp1, ['introduce bug', 'anade comentario'])

    cp2_test_ok = contains_any(cp2, ['python contador.py 5']) and contains_any(cp2, ['good', 'bad', '6', '7'])

    cp3_start_ok = contains_any(cp3, ['git bisect start'])
    cp3_good_bad_ok = contains_any(cp3, ['git bisect good']) and contains_any(cp3, ['git bisect bad'])
    cp3_first_bad_ok = contains_any(cp3, ['first bad commit', 'es el primer commit malo', 'primer commit'])
    cp3_reset_ok = contains_any(cp3, ['git bisect reset'])

    cp4_run_ok = contains_any(cp4, ['git bisect run']) and contains_any(cp4, ['./test_contador.sh', 'test_contador.sh'])
    cp4_result_ok = contains_any(cp4, ['first bad commit', 'commit culpable', 'resultado final'])

    contador_min_content_ok = contains_any(contador, ['def siguiente']) and contains_any(contador, ['return n +'])

    return {
        'cp1_historial_bisect': {
            'ok': cp1_historial_ok,
            'source': 'cp1-historial-bisect.txt' if cp1_path.exists() else 'missing',
        },
        'cp2_prueba_manual_good_bad': {
            'ok': cp2_test_ok,
            'source': 'cp2-prueba-manual.txt' if cp2_path.exists() else 'missing',
        },
        'cp3_proceso_bisect_manual': {
            'ok': cp3_start_ok and cp3_good_bad_ok and cp3_first_bad_ok and cp3_reset_ok,
            'source': 'cp3-proceso-bisect.txt' if cp3_path.exists() else 'missing',
            'subchecks': {
                'start': cp3_start_ok,
                'good_bad': cp3_good_bad_ok,
                'first_bad': cp3_first_bad_ok,
                'reset': cp3_reset_ok,
            },
        },
        'cp4_bisect_run_opcional': {
            'present': cp4_path.exists(),
            'ok': cp4_run_ok and cp4_result_ok,
            'source': 'cp4-bisect-run.txt' if cp4_path.exists() else 'missing',
        },
        'contador_final': {
            'exists': contador_path.exists(),
            'minimum_content_ok': contador_min_content_ok,
        },
        'test_script': {
            'exists': test_script_path.exists(),
        },
    }
...
def main():
    ...
    ap.add_argument('--required', default='contador.py,cp1-historial-bisect.txt,cp2-prueba-manual.txt,cp3-proceso-bisect.txt,reflexion-6-12.md')
    ...
    bisect_info = analyze_bisect_activity(root)
    ...
    reflex_path = root / 'reflexion-6-12.md'
    ...
    evidencia_candidates = [
        'cp1', 'cp2', 'cp3', 'cp4',
        'bisect', 'good', 'bad', 'first bad commit', 'reset', 'reflexion',
    ]
    ...
    for cand in evidencia_candidates:
        for p in root.rglob(f'*{cand}*'):
            if outdir.name in p.parts or '.git' in p.parts:
                continue
            if p.is_file() and p.suffix.lower() in ('.md', '.png', '.jpg', '.jpeg', '.txt', '.sh', '.py', ''):
    ...
    manual_checks = [
        bisect_info['cp1_historial_bisect']['ok'],
        bisect_info['cp2_prueba_manual_good_bad']['ok'],
        bisect_info['cp3_proceso_bisect_manual']['ok'],
        bisect_info['contador_final']['exists'] and bisect_info['contador_final']['minimum_content_ok'],
    ]
    manual_hits = sum(1 for x in manual_checks if x)
    ...
    if bisect_info['cp4_bisect_run_opcional']['ok']:
        s_bisect_run = 3
    elif bisect_info['cp4_bisect_run_opcional']['present']:
        s_bisect_run = 1
    else:
        s_bisect_run = 0
    ...
    total = s_bisect_manual + s_bisect_run + s_reflex
    ...
    resumen = {
        ...
        'scores': {
            'bisect_manual (max_4)': s_bisect_manual,
            'bisect_run_opcional (max_3)': s_bisect_run,
    ...
    with (outdir / 'metricas.csv').open('w', newline='', encoding='utf-8') as f:
        ...
        w.writerow(['bisect_manual (max 4)', s_bisect_manual])
        w.writerow(['bisect_run_opcional (max 3)', s_bisect_run])
    ...
    md.append('|---|---:|')
    md.append(f"| Uso de git bisect manual (start, good, bad, reset) | {s_bisect_manual}/4 |")
    md.append(f"| Uso opcional de git bisect run | {s_bisect_run}/3 |")
    md.append(f"| Reflexion 6.12 | {s_reflex}/3 |")
    ...
    md.append('## Checks de bisect')
    md.append(f"- Historial base (cp1): {'si' if bisect_info['cp1_historial_bisect']['ok'] else 'no'}  ")
    md.append(f"- Prueba manual good/bad (cp2): {'si' if bisect_info['cp2_prueba_manual_good_bad']['ok'] else 'no'}  ")
    md.append(f"- Proceso manual completo (cp3): {'si' if bisect_info['cp3_proceso_bisect_manual']['ok'] else 'no'}  ")
    md.append(f"- Uso opcional de bisect run (cp4): {'si' if bisect_info['cp4_bisect_run_opcional']['ok'] else 'no'}  ")
    md.append(f"- contador.py final presente y minimo valido: {'si' if bisect_info['contador_final']['exists'] and bisect_info['contador_final']['minimum_content_ok'] else 'no'}  ")
    md.append(f"- test_contador.sh detectado (informativo): {'si' if bisect_info['test_script']['exists'] else 'no'}  ")
    md.append('')
